                ...
                tr.innerHTML += `
                    <td>${financialInfo ? `R$ ${financialInfo.provider_payment.toLocaleString('pt-BR')}` :
                        `<input type="number" step="0.01" class="form-control form-control-sm provider-payment" 
                               ...
                               <span class="provider-percentage" style="font-size: 80%;">${financialInfo ? `${financialInfo.provider_percentagem.toLocaleString('pt-BR')} % da diligência` : '0 %'} da diligência</span>
                    </td>
...
function calculateProfit(input) {
    ...
    if (diligenceValueInput && providerPaymentInput && profitCell && providerPercentageCell) {
        ...
        providerPercentageCell.textContent = `${provider_percentagem.toLocaleString('pt-BR')}`;
